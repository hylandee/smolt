...
pub async fn get_workout_sessions() -> Result<Vec<WorkoutSession>, ServerFnError> {
    ...
    let rows = sqlx::query_as::<_, WorkoutSession>(
        "SELECT id, workout_type, created_at, notes \
...
pub async fn get_workout_detail(session_id: i64) -> Result<Option<WorkoutDetail>, ServerFnError> {
    ...
    let sets = sqlx::query_as::<_, ExerciseSet>(
        "SELECT id, session_id, exercise_name, set_number, reps, weight, completed \
         FROM exercise_sets \
         WHERE session_id = ? \
...
pub async fn start_workout(
    ...
    if workout_type != "A" && workout_type != "B" {
        return Err(ServerFnError::new("workout_type must be A or B"));
    }

    let exercises = workout_exercises(&workout_type);
    ...
    let session_id: i64 = sqlx::query_scalar(
        "INSERT INTO workout_sessions (workout_type, notes) VALUES (?, ?) RETURNING id",
    )
    .bind(&workout_type)
    .bind(notes_opt)
    .fetch_one(&pool)
...
pub async fn update_set_weight(
    ...
    sqlx::query("UPDATE exercise_sets SET weight = ? WHERE id = ?")
        .bind(weight)
        .bind(set_id)
        .execute(&pool)
        .await
...
pub async fn get_lift_history(
    ...
    // Return the maximum weight per session for this exercise
    let rows: Vec<(String, f64)> = sqlx::query_as(
        "SELECT ws.created_at, MAX(es.weight) as weight \
         FROM exercise_sets es \
         JOIN workout_sessions ws ON es.session_id = ws.id \
         WHERE es.exercise_name = ? AND es.completed = 1 \
         GROUP BY es.session_id, ws.created_at \
